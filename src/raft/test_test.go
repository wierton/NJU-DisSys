...
func TestReElection(t *testing.T) {
  ...
  util.Log("disconnect %d and re-election\n", leader1)
  cfg.disconnect(leader1)
  cfg.checkOneLeader()

  // if the old leader rejoins, that shouldn't
  // disturb the old leader.
  util.Log("connect %d and re-election\n", leader1)
  cfg.connect(leader1)
  leader2 := cfg.checkOneLeader()

  // if there's no quorum, no leader should
  // be elected.
  util.Log("disconnect %d, %d and re-election\n", leader2,
    (leader2 + 1) % servers)
...
func TestBackup(t *testing.T) {
  ...
  util.Log("submit lots of commands that won't commit\n")
  ...
  util.Log("lots of successful commands to new group.\n")
  // lots of successful commands to new group.
  for i := 0; i < 50; i++ {
    cfg.one(rand.Int() % 100, 3)
  }

  util.Log("now another partitioned leader and one follower\n")
